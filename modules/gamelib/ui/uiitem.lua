...
function UIItem:onMouseRelease(mousePosition, mouseButton)
	...
	if modules.client_options.getOption("classicControl") and not g_app.isMobile() and (g_mouse.isPressed(MouseLeftButton) and mouseButton == MouseRightButton or g_mouse.isPressed(MouseRightButton) and mouseButton == MouseLeftButton) then
		...
		return true
	elseif modules.game_interface.processMouseAction(mousePosition, mouseButton, nil, item, item, nil) then
		return true
	end
...
function UIItem:getItemTooltip()
	if self:getItem() then
		return self:getItem().itemTooltip
	end
end

function UIItem:setItemTooltip(tooltip)
	if self:getItem() then
		self:getItem().itemTooltip = tooltip
	end
end
