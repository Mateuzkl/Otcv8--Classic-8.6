...
function UIMinimap:load()
	...
	if settings then
		...
		local zoom = tonumber(settings.zoom) or 0
		zoom = math.floor(math.max(-5, math.min(5, zoom)))
		-- print("UIMinimap:load - zoom value: " .. tostring(zoom))
		pcall(function() self:setZoom(zoom) end)
	end
end
...
function UIMinimap:onZoomChange(zoom)
	for _, widget in pairs(self.alternatives) do
		if (not widget.minZoom or zoom <= widget.minZoom) and widget.maxZoom <= zoom then
			widget:show()
		else
			widget:hide()
		end
...
function UIMinimap:onMouseRelease(pos, button)
	...
	if button == MouseLeftButton then
		...
		if self.autowalk and not player:isWalkLocked() then
	...
	elseif button == MouseRightButton then
		...
		menu:setGameMenu(true)
		menu:addOption(tr("Set mark"), function ()
