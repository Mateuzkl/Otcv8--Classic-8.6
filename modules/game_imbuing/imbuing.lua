local imbuingWindow = nil
local bankGold = 0
local inventoryGold = 0
local itemImbuements = {}
local emptyImbue, groupsCombo, imbueLevelsCombo, protectionBtn, clearImbue, selectedImbue = nil
local imbueItems = {}
local protection = false
local clearConfirmWindow, imbueConfirmWindow = nil
...
function init()
	...
	function groupsCombo.onOptionChange(widget)
		...
		if itemImbuements ~= nil then
			...
			imbueLevelsCombo:onOptionChange()
		end
	end

	function imbueLevelsCombo.onOptionChange(widget)
		setProtection(false)

		local selectedGroup = groupsCombo:getCurrentOption().text

		for _, imbuement in ipairs(itemImbuements) do
			if imbuement.group == selectedGroup and #imbuement.sources == widget.currentIndex then
				...
				for i, source in ipairs(imbuement.sources) do
					for _, item in ipairs(imbueItems) do
						if item:getId() == source.item:getId() then
							if source.item:getCount() <= item:getCount() then
				...
				if not protection and imbuement.cost <= bankGold + inventoryGold then
	...
	function protectionBtn.onClick()
...
function selectSlot(widget, slotId, activeSlot)
	if activeSlot then
		...
		function clearImbue.clear.onClick()
			imbuingWindow:hide()
	...
	function emptyImbue.imbue.onClick()
		imbuingWindow:hide()
...
function onImbuementWindow(itemId, slots, activeSlots, imbuements, needItems)
	...
	for i, slot in pairs(activeSlots) do
		...
		function activeSlotBtn.onClick(widget)
