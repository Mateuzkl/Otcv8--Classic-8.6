...
function onTradeClick()
	...
	if getCurrentTradeType() == BUY then
		g_game.buyItemWithServerId(selectedItem.ptr, selectedItem.serverId, quantityScroll:getValue(), ignoreCapacity:isChecked(), buyWithBackpack:isChecked())
	else
		g_game.sellItemWithServerId(selectedItem.ptr, selectedItem.serverId, quantityScroll:getValue(), ignoreEquipped:isChecked())
	end
end
...
function itemPopup(self, mousePosition, mouseButton)
	...
	if mouseButton == MouseRightButton then
		...
		menu:addOption(tr("Look"), function ()
			return g_game.inspectNpcTradeWithServerId(self:getItem(), self.serverId)
		end)
	...
	elseif g_mouse.isPressed(MouseLeftButton) and mouseButton == MouseRightButton or g_mouse.isPressed(MouseRightButton) and mouseButton == MouseLeftButton then
		...
		g_game.inspectNpcTradeWithServerId(self:getItem(), self.serverId)
...
function getItemPrice(item, single)
	...
	if getCurrentTradeType() == BUY and buyWithBackpack:isChecked() then
...
function getSellQuantity(item, serverId)
...
function canTradeItem(item)
	if getCurrentTradeType() == BUY then
		return (ignoreCapacity:isChecked() or not ignoreCapacity:isChecked() and item.weight <= playerFreeCapacity) and getItemPrice(item, true) <= playerMoney
	else
...
function refreshTradeItems()
	...
	for key, item in pairs(currentTradeItems) do
		...
		itemWidget.serverId = item.serverId
...
function onPlayerGoods(money, items)
	...
	for key, item in pairs(items) do
		...
		if not playerItems[id] then
			playerItems[id] = item[3]
		else
			playerItems[id] = playerItems[id] + item[3]
		end
	end
...
function formatCurrency(amount)
	if CURRENCY_DECIMAL then
		return string.format("%.02f", amount / 100) .. " " .. CURRENCY
	else
...
function sellAll(delayed, exceptions)
	...
	for _, entry in ipairs(tradeItems[SELL]) do
		...
		if not table.find(exceptions, id) then
			...
			while sellQuantity > 0 do
				...
				if delayed then
					g_game.sellItemWithServerId(entry.ptr, entry.serverId, maxAmount, ignoreEquipped:isChecked())
	...
	for _, entry in ipairs(queue) do
		g_game.sellItemWithServerId(entry[1], entry[2], entry[3], entry[4])
	end
end
